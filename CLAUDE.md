...
## Current Work
- **Last updated**: 2026-06-29 — **4 Rankings features shipped: Top-Models-by-Task treemap, two-column LLM leaderboard, Others→pink, slug-collision fix. DEPLOYED; pink + 2-col live-verified; treemap awaits a KV refresh.**
- **State**: deployed — **Prod Version `66ca3da6-3975-4031-98f0-3c28caa4e7c4`**. feat `fa36fd5` on `main` (origin synced; the prior `4ef6b84`+`2e1fab8` were pushed at the start of this session). Bindings (KV/D1/`BROWSER`) + cron `0 * * * *` + `REFRESH_SECRET` untouched.
- **#2 Treemap** (`template.ts` client JS + `openrouter-json.ts`): squarified (Bruls), nested by macro-category, click a tile → its top-10 models (2-col). Source = `task-spend` **spend** half. See the new Gotcha. Legend matches OpenRouter (Gen 35.5 / Agent 30.6 / Code 26.5 / Data 7.4).
...
- **#5 Slug fix**: `fetchModelBoard` groups by `canonModel` (sums dated/`:free` variants into one row). 2 real collisions confirmed in the live `/models` feed. Effective once the next refresh repopulates RANKINGS KV.
- **Post-deploy**: treemap (`TASK_SPEND` KV) + deduped leaderboard light up on the next hourly cron (~:00) or a manual `POST /api/refresh` (fast path now includes task-spend — look for `tasks: 30`). Until then the treemap shows a graceful "refreshes hourly" message; pink + 2-col are already live.
