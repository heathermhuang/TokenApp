...
## Current Work
- **Last updated**: 2026-06-26 — **Token Share chart rewrite + rankings JSON migration. Code merged to `main`; PROD DEPLOY PENDING** (blocked by a transient CC classifier outage — just retry `npx wrangler deploy`).
- **State**: `main` @ `73450f6` (merge commit; 8 commits this session, `feature/rankings-json-share-chart` kept). NOT yet pushed to origin; NOT yet deployed. Spec + plan in `docs/superpowers/{specs,plans}/2026-06-26-token-share-timeseries-rankings-json*`.
...
- **NEXT (resume here)**: 1) `npx wrangler deploy` (retry past classifier flakiness). 2) After deploy KV holds the OLD shape until refresh — chart 404s + agent 7D/30D empty until the next hourly cron OR you run `POST /api/refresh` with `REFRESH_SECRET`. 3) Smoke: `/api/market-share` → `{author,model}` 52wk; `/api/rankings?period=week` → apps.week=20; home shows "Token Share Over Time". 4) `git push origin main`. 5) `/codex` review.
