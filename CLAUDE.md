...
- **Shipped this session** (deployed, prod version `8325ef68`): `6f36211` /api/refresh awaits fast path + categories cron-only (supersedes a broken `waitUntil`+202 — see new gotchas) · `234f2ad` market-share extract-by-content + slug filter (the real empty-market-share fix) · `d234b54` read-side author sanitizer · `9e3e20a` codex P2 fixes (poll mirrors extractor; `historyDays` from filtered set). Earlier intermediate commits `a1373b0`/`a519092` superseded but kept in branch history. **Whole branch codex-reviewed clean + tsc-clean.**
