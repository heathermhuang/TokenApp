...
- **What shipped this session**: two deploys to prod.
  ...
  - `d648a4a` `feat(agent-readiness): OAuth Protected Resource Metadata (RFC 9728)` — `/.well-known/oauth-protected-resource` declaring token.app as a resource server whose authorization_servers is itself and whose only protected endpoint is `/api/refresh`. `POST /api/refresh` 401s now emit `WWW-Authenticate: Bearer realm=..., scope="admin:refresh", resource_metadata="..."` per RFC 9728 §5.1 so clients can auto-discover the auth flow.
...
- **Local state**: on `main` at `d648a4a`, clean apart from this CLAUDE.md update and untracked `.claude/`. Nothing stashed.
