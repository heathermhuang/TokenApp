...
## Current Work
- **Last updated**: 2026-06-27 — **Polished data-dense refresh DEPLOYED to prod & verified live.** Site-wide visual pass: harmonized chart palette + tabular figures + calmer leaderboards.
- **State**: deployed — **Prod Version `27122b63-a6d6-49c2-8896-c47ab1211b47`** (`wrangler deploy` run from the Claude env; wrangler is authenticated here). `main` (`85f24d6`) pushed → `origin/main` in sync. Verified live: token.app serves `CHART_PALETTE_DARK`/`chartColor`/`chartSlot`; old `genHue`/`brandColor`/`/s+/g` gone. Bindings (KV/D1/`BROWSER`) + cron `0 * * * *` + `REFRESH_SECRET` untouched.
- **What shipped (`24dcaad..a89870c`)**: spec `docs/superpowers/specs/2026-06-27-polished-data-dense-refresh-design.md`. Code (`src/template.ts`): (1) harmonized **entity-stable** chart palette — `CHART_PALETTE_{DARK,LIGHT}` + `CHART_SLOT` map; `chartColor`/`chartSlot` **replace** `brandColor`/`genHue`; **decoupled from brand chips** (`PROVIDER_STYLE_*` untouched → pricing/subs chips unchanged); `modelTint` derives from `chartColor`. (2) 1px surface band separators, fill-opacity 0.9. (3) sparkline → single muted hue (`var(--text2)`); direction stays in ▲/▼ chip. (4) body `font-variant-numeric: tabular-nums` + font smoothing. (5) micro-caps `.stat-label`.
- **Bug fixed (latent)**: template-literal `\s` trap — slug regex emitted as `/s+/g`, mangling any slug with "s" (`deepseek`→`deep-eek`) → hash-fallback color collisions. Now a literal-space regex in `chartSlot`. Harmless under the old `genHue` code; only bit once exact slug→slot mapping landed.
- **Verify harness (gitignored `scratchpad/`)**: `verify.mjs` (esbuild-render `getHtml()` + vm syntax-check, 0 errors + `preview-{dark,light}.html`); `shoot.py` + ad-hoc `dump_colors.py` (playwright, both themes, no console errors, all entities distinct slots).
- **TOOLCHAIN**: wrangler "hang" = corrupted `miniflare` in `node_modules` (`miniflare.Mutex` undefined); **fix = `npm ci`**.
- **Backlog**: [P2] canonical model slug collision (`fetchModelBoard`). Chart palette: only the hash *fallback* can collide — if a real top-set shows two same-colored unmapped authors, add them to `CHART_SLOT` (the 9 current top brands map to 9 distinct slots). Real category-board JSON. Optional CI `.github/workflows/deploy.yml` (untracked) + `CLOUDFLARE_API_TOKEN` secret.
