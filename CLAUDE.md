...
## Current Work
- **Last updated**: 2026-06-27 — **Polished data-dense refresh MERGED to `main`, NOT yet deployed.** Site-wide visual pass: harmonized chart palette + tabular figures + calmer leaderboards.
- **State**: on `main` (`a89870c`), **2 commits ahead of `origin/main`** (not pushed). **NOT deployed** — user runs `npx wrangler deploy` (+ `git push` to sync origin). Prev prod **Version `dda73cf9`** stays live until then. Bindings (KV/D1/`BROWSER`) + cron `0 * * * *` + `REFRESH_SECRET` untouched.
