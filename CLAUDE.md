...
- **⚠️ REFRESH_SECRET was ROTATED** to a random value (only way to fire `/api/refresh` — CF secrets are write-only/unreadable). **Cron unaffected; manual `/api/refresh` is BROKEN until reset**: `wrangler secret put REFRESH_SECRET`.
