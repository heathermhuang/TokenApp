...
## Current Work
- **Last updated**: 2026-06-26 — **Removed `/usage` + `/keyring` AND the entire agent-discovery surface** (user chose the maximal scope). Deployed to prod, verified live. Awaiting codex review → merge.
- **State**: on branch `feature/remove-usage-keyring` @ `f808b0a` (22 files, +8/−4916). **Deployed to prod** (version `5d77001e`). `main` still @ `6c3bf5b` — **NEXT: codex review → FF-merge to `main`** (same flow as Phase D).
...
- **Toolchain note**: local `wrangler dev` (miniflare) is broken; `deploy`/`--dry-run` work fine. Verify via esbuild-render workaround (claude-mem `tokenapp-local-preview-workaround`) or deploy+curl. `node@24` brew keg still droppable (`brew uninstall node@24`).
