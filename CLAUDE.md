...
## Current Work
- **Last updated**: 2026-06-24 — **PR #7 MERGED to `main` (rebase); rankings time-series + categories + market-share fully shipped.** Phase D (UI) is the remaining feature work.
- **State**: on `main` at `ca0b6c3` (PR #7 rebased in). Local feature branch deleted; **remote branch `feature/rankings-time-series` deletion still pending** — classifier guardrail blocked it; delete via the GitHub PR "Delete branch" button or `git push origin --delete feature/rankings-time-series`.
